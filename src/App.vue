   ...
   <div class="fixed w-100 h-100 opacity-80 bg-purple-800 inset-0 z-50 flex items-center justify-center">
    ...
    <div class="container">
      ...
      <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
        <div
            v-for="(t, indx) in paginatedTickers"
            :key="indx"
      ...
      <section v-if="sel" class="relative">
        ...
        <div
            ...
            <div v-for="(bar, indx) in normalizedGraph"
                 :key="indx"
                 :style="{height: `${bar}%`}"
...
export default {
  ...
  methods:{
    calculateMaxGraphElements(){
      ...
      this.maxGraphElemetcs = this.$refs.graph.clientWidth /38
    },
    updateTicker(tickerName, price){
      this.tickers.filter(t => t.name === tickerName).forEach(t =>{
        if(t.name === this.sel?.name){
         this.graph.push(price)
          while(this.graph.length > this.maxGraphElemetcs){
            this.graph.shift()
          }
          /*if(this.graph.length > this.maxGraphElemetcs){
            this.graph = this.graph.slice(0, this.maxGraphElemetcs)
          }*/
       }
        t.price = price
      })
    ...
    removeTicker(tickersToRemove){
     this.tickers = this.tickers.filter(item => item !== tickersToRemove)
      this.sel = null
      unsubscribeFromTicker(tickersToRemove.name)
    },
